...
export default function WikiLayout({ children, breadcrumbs = [], sidebarTree, universe, toc, wide = false }: Props) {
    const { cssVars, iconUrl, themeColor } = useUniverseTheme(universe);

    return (
        <div
            className="flex min-h-screen flex-col bg-white font-sans text-slate-900 antialiased selection:bg-blue-100 selection:text-slate-900"
            style={cssVars}
        >
            <WikiNavbar
            ...
            <div className="flex flex-1">
                ...
                <main className={`mx-auto w-full flex-1 px-6 py-10 animate-fade-in-up sm:max-w-3xl md:max-w-6xl lg:max-w-7xl`}>
                    {children}
                </main>
            </div>
